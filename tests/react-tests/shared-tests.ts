...
export function runReactTests(packageManager: "bun" | "pnpm" | "npm") {
  const projectName = "testing-react"
  let tempDir = ""
  let projectPath = ""

  describe.sequential(`${packageManager} react tests`, () => {
    beforeAll(async () => {
    ...
    it("should install dependencies", async () => {
      const exists = await fs.access(path.join(projectPath, "node_modules")).then(() => true).catch(() => false)
      expect(exists).toBe(true)
    })

    it("should have config files", async () => {
      const hasPrettier = await fs.access(path.join(projectPath, ".prettierrc.json")).then(() => true).catch(() => false)
      const hasPush = await fs.access(path.join(projectPath, "push.sh")).then(() => true).catch(() => false)
      expect(hasPrettier && hasPush).toBe(true)
    })

    it("should include tailwind import in vite config", async () => {
    ...
    it("should add tailwind directive in index.css", async () => {
    ...
    it("should delete app.css", async () => {
      const exists = await fs.access(path.join(projectPath, "src/app.css")).then(() => true).catch(() => false)
      expect(exists).toBe(false)
    })

    it("should delete assets", async () => {
      const exists = await fs.access(path.join(projectPath, "src/assets")).then(() => true).catch(() => false)
      expect(exists).toBe(false)
    })

    it("should delete eslint.config.js", async () => {
      const exists = await fs.access(path.join(projectPath, "eslint.config.js")).then(() => true).catch(() => false)
      expect(exists).toBe(false)
    })

    it("should clear app.tsx content", async () => {
