export const projects = ["react-ts-tw", "next-prisma", "wxt", "uv-notebook", "cmake-cpp"] as const
export type TprojectType = (typeof projects)[number]

export const packageManagers = ["pnpm", "npm", "bun"] as const
export type TpackageManager = (typeof packageManagers)[number]

export const openInEditorOptions = ["antigravity", "devin-desktop", "subl", "no", "cursor", "code"] as const
export type TopenInEditor = (typeof openInEditorOptions)[number]

export const wxtTemplates = ["svelte", "vanilla", "solid"] as const
export type TwxtTemplatesType = (typeof wxtTemplates)[number]
